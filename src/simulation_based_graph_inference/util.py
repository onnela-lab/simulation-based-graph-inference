...
def _plot_generated_graph(
    generator: typing.Callable, *args, num_nodes: int = 20
) -> None:
    """
...
def assert_interval(
    ...
    if outside:
        raise ValueError(
            f"{name} must belong to the interval {'[' if inclusive_low else '('}"
...
def clustering_coefficient(
    edge_index: th.Tensor, num_nodes: typing.Optional[int] = None
) -> th.Tensor:
    """
    ...
    denominator = (
        th.bincount(edge_index[0], minlength=num_nodes)
        * th.bincount(edge_index[1], minlength=num_nodes)
        - reciprocated
    )
