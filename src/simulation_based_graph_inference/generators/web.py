...
def web_graph(
    ...
    proba_new = assert_interval(
        "proba_new", proba_new, 0, 1, inclusive_low=False, dtype=float
    )
    proba_uniform_new = assert_interval(
        "proba_uniform_new", proba_uniform_new, 0, 1, dtype=float
    )
    proba_uniform_old1 = assert_interval(
        "proba_uniform_old1", proba_uniform_old1, 0, 1, dtype=float
    )
    proba_uniform_old2 = assert_interval(
        "proba_uniform_old2",
        proba_uniform_old1 if proba_uniform_old2 is None else proba_uniform_old2,
        0,
        1,
        dtype=float,
    )
